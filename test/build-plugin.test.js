...
test('builds a fastify plugin', t => {
  ...
  t.equal(opts.fastify, '>= 2.x', 'adds option for fastify support')
...
test('plugin', t => {
  ...
  t.test('no namespace', async t => {
    ...
    t.ok(decorate.calledWith('secrets', {
  ...
  t.test('no namespace - secrets exists', async t => {
    ...
    const promise = plugin({ decorate, secrets: {} }, {
      secrets: {
        secret1: 'secret1-name',
        secret2: 'secret2-name'
      }
    ...
    await t.rejects(
      promise,
      new Error('fastify-secrets has already been registered'),
      'registration fails'
     )
  ...
  t.test('no options', async t => {
    ...
    await t.rejects(
      promise,
      new Error(`fastify-secrets: no secrets requested`),
      'registration fails'
    )
    t.notOk(decorate.called, 'does not decorate fastify')
  })
