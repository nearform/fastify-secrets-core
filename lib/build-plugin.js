...
function buildPlugin(Client, pluginOpts) {
  async function FastifySecretsPlugin(fastify, opts) {
    ...
    const clientLabel = namespace ? `${namespace}SecretClient` : `secretClient`
    fastify.decorate(clientLabel, async (refs) => {
      ...
      const refreshedSecrets = await getSecretsFromClient(activeClient, concurrency, secretsToRefresh)
    ...
    decorateWithSecrets(fastify, namespace, secrets)
